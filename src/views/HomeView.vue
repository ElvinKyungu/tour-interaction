...
const messages = [
  ...
  },
  {
    ...
    title: 'Task Assigned!',
    description: 'A new task is waiting for your action.',
    textLeft: 'Yesterday',
  },
]
...
const animateListItems = () => {
  ...
  if (items) {
    animate(
      ...
      { delay: stagger(0.2), duration: 0.5, easing: 'ease-in-out' }
    )
  }
}
...
<template>
  <main class="w-full h-screen flex justify-center items-center">
    <div ref="containerRef" class="container mx-auto max-w-xl border rounded-lg p-5">
      ...
      <div class="flex justify-between items-center cursor-pointer" @click="openContainer">
        <div class="flex gap-5 items-center">
          ...
          <div class="flex flex-col">
            ...
            <span class="transition-all text-gray-500" :class="{'hidden transition-all': isOpen}">What happening around you?</span>
          </div>
        </div>
      ...
      <!-- Notifications -->
      <ul class="overflow-hidden transition-all mt-5 space-y-4" 
          ...
          <div class="flex justify-between items-center">
            <div class="flex gap-5 items-center">
              <div class="p-3 rounded-lg bg-slate-100">
                <component :is="message.icon" class="w-6 h-6" />
              </div>
              <div class="flex flex-col">
                <span class="text-base font-medium">{{ message.title }}</span>
